...
if [ "$CLOUD_MODE" = true ]; then
  echo "Cloud mode activated. Ollama will be used for cloud models (gpt-oss:120b-cloud)."
  # No local model pull or waiting for client-side model here in cloud mode
  # The agent's code is responsible for making requests to the cloud model directly
else
  echo "Local mode activated. Pulling and waiting for gemma3 model."
  # Pull the gemma3 model if not already present
  if ! ollama list | grep -q "gemma3"; then
    echo "Pulling gemma3 model..."
    ollama pull gemma3
  fi

  # Wait for Ollama to be available and gemma3 to be detected
